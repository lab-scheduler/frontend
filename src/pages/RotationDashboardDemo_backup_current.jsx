...
export default function RotationDashboard(){
  ...
  const todayStart = isoDate(startOfMonth(today))
  const todayEnd = isoDate(endOfMonth(today))
  const monthStart = isoDate(startOfMonth(calendarDate))
  const monthEnd = isoDate(endOfMonth(calendarDate))
  ...
  // Load dashboard data (only once and for current month)
  useEffect(()=>{
    let mounted = true
  ...
  },[token, todayStart, todayEnd]) // Only load once for dashboard

  // Load calendar shifts separately when month changes
  useEffect(()=>{
    let mounted = true
    async function loadCalendarShifts(){
      try{
        const shiftsP = apiFetch(`/api/v1/${ORG_SLUG}/shifts?start_date=${monthStart}&end_date=${monthEnd}`, {}, token)
        const res = await Promise.allSettled([shiftsP])
        if(!mounted) return

        const shiftsVal = res[0].status==='fulfilled'? res[0].value : []
        setCalendarShifts(Array.isArray(shiftsVal) ? shiftsVal : [])
      }catch(err){
        console.error('Error loading calendar shifts:', err)
      }
    }
    loadCalendarShifts()
    return ()=> mounted=false
  },[token, monthStart, monthEnd]) // Reload when month changes
  ...
  const todayShifts = useMemo(() => {
    ...
    console.log('Today shifts filtered:', shifts) // Debug log
    return shifts
  ...
  function openDayModal(dateKey){
    const dayShifts = scheduleMap[dateKey] || (analysisShifts.filter(s=>(s.shift_date||s.date||'').startsWith(dateKey)))
  ...
  async function runEnhancedScheduler(params) {
    ...
    try {
      ...
      const shiftsCheck = await apiFetch(
        `/api/v1/${ORG_SLUG}/shifts?start_date=${start_date}&end_date=${end_date}`,
        {},
        token
      )
      ...
      const response = await apiFetch(`/api/v1/${ORG_SLUG}/schedule/run-enhanced`, {
        method: 'POST',
        headers: {
      ...
      // Optionally refresh data after successful run
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
        ...
        {/* Analytics Overview */}
        <div className="bg-white rounded-lg shadow-sm p-6" style={{ height: '800px' }}>
          <h3 className="font-semibold text-lg mb-4">Analytics Overview</h3>

          {/* Quality Metrics */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="bg-blue-50 border-2 border-blue-200 rounded-lg p-4">
              <div className="text-sm font-semibold text-blue-800">Compliance Rate</div>
              <div className="text-2xl font-bold text-blue-900">
                { quality.compliance_rate ? `${Number(quality.compliance_rate).toFixed(1)}%` : '—' }
              </div>
            </div>

            <div className="bg-amber-50 border-2 border-amber-200 rounded-lg p-4">
              <div className="text-sm font-semibold text-amber-800">Supervisor Coverage</div>
              <div className="text-2xl font-bold text-amber-900">
                { quality.supervisor_coverage ? `${Number(quality.supervisor_coverage).toFixed(1)}%` : '—' }
              </div>
            </div>
          ...
          <div>
            <h4 className="font-medium text-gray-700 mb-3">Optimization Opportunities</h4>
            { (report?.optimization_opportunities || []).length ? (
              <div className="space-y-3 overflow-y-auto" style={{ maxHeight: '300px' }}>
                {report.optimization_opportunities.map((op,i)=>(
                  <div key={i} className="p-3 border rounded-lg">
                    <div className="font-medium text-gray-900">{op.area}</div>
                    <div className="text-sm text-gray-600 mt-1">{op.implementation}</div>
                    <div className="text-sm text-indigo-600 font-semibold mt-2">{op.potential_savings}</div>
                    <div className="mt-2 flex gap-2">
                      <button className="px-3 py-1 text-xs border rounded hover:bg-gray-50">Simulate</button>
                      <button className="px-3 py-1 text-xs bg-indigo-600 text-white rounded hover:bg-indigo-700">Apply</button>
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <div className="text-sm text-gray-500 text-center py-8">No optimization opportunities suggested</div>
            )}
          </div>
        </div>
      </div>
              ...
              {!schedulerResult ? (
                <form onSubmit={(e) => {
                  ...
                  runEnhancedScheduler({
                    ...
                    use_cpsat: formData.get('use_cpsat') === 'true',
                    cpsat_time: parseInt(formData.get('cpsat_time')) || 30
                  })
                }} className="space-y-4">
                  <div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    >
                      <option value="false">Standard (Faster)</option>
                      <option value="true">CP-SAT (More Optimal)</option>
                    </select>
                  </div>
                  ...
                  {schedulerResult.success !== false ? (
                    <div>
                      ...
                      <div className="bg-gray-50 p-3 rounded text-sm">
                        <p className="text-gray-600">The page will refresh automatically to show the updated schedule.</p>
                      </div>
                    </div>
                  ) : (
                    <div>
