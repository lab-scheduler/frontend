...
export default function RotationDashboard(){
  ...
  const todayShifts = useMemo(() => {
    ...
    if (shifts.length > 0) {
      console.log('Sample shift structure:', shifts[0])
      console.log('Department field:', shifts[0].department)
      console.log('All fields:', Object.keys(shifts[0]))
    }
    return shifts
                    ...
                    <div key={i} className="border rounded-lg overflow-hidden hover:shadow-md transition-shadow">
                      ...
                      <div className="px-4 py-2 bg-indigo-50 border-b">
                        <div className="font-semibold text-indigo-700">
                          {(() => {
                            // Debug logging for department fields
                            if (import.meta.env.DEV) {
                              console.log(`Shift ${i} department fields:`, {
                                's.department?.name': s.department?.name,
                                's.department_name': s.department_name,
                                's.department': s.department,
                                's.dept_id': s.dept_id,
                                's.department_id': s.department_id,
                                'typeof s.department': typeof s.department,
                                'All shift keys': Object.keys(s)
                              });
                            }
                            return s.department?.name || s.department_name || (typeof s.department === 'string' ? s.department : 'Department');
                          })()}
                        </div>
                      </div>
